...
app.post("/api/process-photo", uploadPhotoAndBg, async (req, res) => {
  try {
    ...
    // 2. Détecter les contours réels du sujet détouré (fond transparent) puis ajouter
    // une marge au-dessus de la tête, pour garantir qu'elle ne soit jamais coupée —
    // au lieu de deviner un recadrage, on se base sur la vraie silhouette de la personne.
    let paddedSubject;
    try {
      const trimmedInfo = await sharp(fgBuffer).trim().toBuffer({ resolveWithObject: true });
      const { width: tw, height: th } = trimmedInfo.info;
      const headroom = Math.round(th * 0.14);
      const sidePad = Math.round(tw * 0.08);
      paddedSubject = await sharp(trimmedInfo.data)
        .extend({ top: headroom, bottom: 0, left: sidePad, right: sidePad, background: { r: 0, g: 0, b: 0, alpha: 0 } })
        .png()
        .toBuffer();
    } catch (trimErr) {
      // Repli si la détection de contour échoue sur une photo particulière
      paddedSubject = fgBuffer;
    }

    // 3. Redimensionner pour remplir le cadre cible, aligné en haut (la tête, marge
    // comprise, est donc toujours entièrement visible ; seul le bas — torse/épaules —
    // peut être rogné si besoin, ce qui est normal sur une photo d'identité).
    const resizedForeground = await sharp(paddedSubject)
      .resize(targetFormat.widthPx, targetFormat.heightPx, { fit: "cover", position: "top" })
      .ensureAlpha()
      .png()
      .toBuffer();

    // 3bis. Adoucir légèrement les bords de la découpe (évite l'effet "collé"/artificiel)
...
app.post("/api/studio-edit", upload.single("photo"), async (req, res) => {
  try {
    ...
    // On force la préservation exacte du visage/identité, quelle que soit la demande de l'utilisateur.
    // Les gros modèles de retouche par texte ont tendance à "réinventer" le visage sur les
    // transformations importantes (tenue, décor) — cette consigne réduit fortement ce risque.
    const identityGuard =
      "Keep the exact same person: identical face, facial features, skin tone, eyes, nose, mouth, " +
      "hairstyle and facial expression, unchanged and unaltered — do not regenerate or beautify the face. " +
      "Only modify what is explicitly requested below, nothing else on the person's face. Request: ";
    const finalPrompt = `${identityGuard}${prompt.trim()}`;
